"use client";
import Link from "next/link";
import { motion } from "framer-motion";
import {
  TrendingUp, Wallet, ArrowDownToLine, AlertCircle,
  ShoppingCart, FilePlus2, Users, Boxes, Sparkles, Calendar, Bell,
} from "lucide-react";
import PageWrapper from "@/components/layout/PageWrapper";
import { PageHeader } from "@/components/ui/PageHeader";
import { StatCard } from "@/components/ui/StatCard";
import { Badge } from "@/components/ui/Badge";
import { CurrencyDisplay } from "@/components/ui/CurrencyDisplay";
import { DashboardSkeleton } from "@/components/skeletons/DashboardSkeleton";
import { RevenueChart } from "@/components/charts/RevenueChart";
import { ExpenseDonut } from "@/components/charts/ExpenseDonut";
import { HealthGauge } from "@/components/charts/HealthGauge";
import { useLoadingSimulation } from "@/lib/hooks/useLoadingSimulation";
import { INVOICES } from "@/lib/mock-data/invoices";
import { TAX_FILINGS } from "@/lib/mock-data/tax";
import { formatDate, daysUntil } from "@/lib/utils/dates";

const QUICK_ACTIONS = [
  { label: "New Invoice",   href: "/sales/new-invoice",         icon: FilePlus2,    color: "gradient-teal" },
  { label: "Run Payroll",   href: "/payroll/run-payroll",       icon: Wallet,       color: "gradient-emerald" },
  { label: "Add Customer",  href: "/crm/customers",             icon: Users,        color: "gradient-gold" },
  { label: "Stock Check",   href: "/inventory/items",           icon: Boxes,        color: "gradient-blue" },
];

const ALERTS = [
  { type: "warning", icon: Bell, title: "12 items below reorder level", description: "Coffee Beans, Cement, Rice Bag", href: "/inventory/items" },
  { type: "danger",  icon: AlertCircle, title: "7 overdue invoices",     description: "Total outstanding: TZS 13.1M",   href: "/sales/invoices" },
  { type: "info",    icon: Calendar, title: "VAT Return due in 4 days",  description: "October 2024 — TZS 17.9M",       href: "/tax/vat-returns" },
];

export default function DashboardPage() {
  const loading = useLoadingSimulation(900);
  if (loading) return <PageWrapper><DashboardSkeleton /></PageWrapper>;

  const recentInvoices = INVOICES.slice(0, 6);

  return (
    <PageWrapper>
      <PageHeader
        title="Good morning, Elia"
        subtitle="Here's how Kilimanjaro Trading is doing today, 27 May 2026."
      />

      {/* KPI ROW */}
      <motion.div
        initial="hidden"
        animate="show"
        variants={{ hidden: {}, show: { transition: { staggerChildren: 0.07 } } }}
        className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6"
      >
        {[
          { label: "Revenue MTD",     value: 84_700_000,  trend: 12.4, icon: <TrendingUp />,       variant: "teal" as const,    prefix: "TSh" },
          { label: "Net Profit MTD",  value: 27_500_000,  trend: 8.1,  icon: <Wallet />,           variant: "emerald" as const, prefix: "TSh" },
          { label: "Cash Position",   value: 312_800_000, trend: -3.2, icon: <ArrowDownToLine />,  variant: "blue" as const,    prefix: "TSh" },
          { label: "Receivables",     value: 89_450_000,  trend: 5.3,  icon: <AlertCircle />,      variant: "amber" as const,   prefix: "TSh", invert: true },
        ].map((kpi) => {
          const invert = "invert" in kpi ? (kpi as { invert: boolean }).invert : undefined;
          return (
            <motion.div key={kpi.label} variants={{ hidden: { opacity: 0, y: 14 }, show: { opacity: 1, y: 0 } }}>
              <StatCard
                label={kpi.label}
                value={kpi.value}
                prefix={kpi.prefix}
                trendValue={kpi.trend}
                {...(invert !== undefined && { trendInvert: invert })}
                icon={kpi.icon}
                variant={kpi.variant}
                format="compact"
              />
            </motion.div>
          );
        })}
      </motion.div>

      {/* Charts row */}
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 mb-6">
        <Section className="lg:col-span-3" title="Revenue • Expenses • Profit" subtitle="Trailing 12 months">
          <RevenueChart />
        </Section>
        <Section className="lg:col-span-2" title="Expense breakdown" subtitle="YTD by category">
          <ExpenseDonut />
        </Section>
      </div>

      {/* Recent + Quick Actions */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        <Section className="lg:col-span-2" title="Recent transactions" subtitle="Latest invoices" headerRight={
          <Link href="/sales/invoices" className="text-xs font-medium text-ud-primary hover:underline">
            View all
          </Link>
        }>
          <div className="divide-y divide-ud-border -mx-5">
            {recentInvoices.map((inv) => (
              <Link
                key={inv.id}
                href="/sales/invoices"
                className="flex items-center gap-3 px-5 py-3 hover:bg-ud-surface-2 transition-colors"
              >
                <div className="w-9 h-9 rounded-xl bg-ud-primary-50 flex items-center justify-center flex-shrink-0">
                  <ShoppingCart className="w-4 h-4 text-ud-primary" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-ud-text-primary truncate">{inv.customerName}</div>
                  <div className="text-xs text-ud-text-muted">{inv.number} · {formatDate(inv.issueDate)}</div>
                </div>
                <div className="text-right">
                  <CurrencyDisplay amount={inv.total} compact className="text-sm font-mono font-medium" />
                  <div className="mt-0.5">
                    <Badge size="sm" variant={
                      inv.status === "Paid"      ? "success" :
                      inv.status === "Overdue"   ? "danger"  :
                      inv.status === "Sent"      ? "info"    :
                      inv.status === "Cancelled" ? "default" : "warning"
                    }>{inv.status}</Badge>
                  </div>
                </div>
              </Link>
            ))}
          </div>
        </Section>

        <Section title="Quick actions" subtitle="Most-used flows">
          <div className="grid grid-cols-2 gap-2.5">
            {QUICK_ACTIONS.map((a) => {
              const Icon = a.icon;
              return (
                <Link
                  key={a.label}
                  href={a.href}
                  className="group p-3 rounded-xl border border-ud-border bg-white hover:border-ud-primary hover:shadow-card-hover transition-all"
                >
                  <div className={`w-9 h-9 rounded-xl ${a.color} flex items-center justify-center mb-2`}>
                    <Icon className="w-4 h-4 text-white" />
                  </div>
                  <div className="text-sm font-medium text-ud-text-primary group-hover:text-ud-primary transition-colors">
                    {a.label}
                  </div>
                </Link>
              );
            })}
          </div>
        </Section>
      </div>

      {/* Health + Tax deadlines + Alerts */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        <Section title="Financial health" subtitle="74/100 — Strong">
          <div className="flex flex-col items-center justify-center">
            <HealthGauge score={74} size={170} />
            <div className="mt-4 grid grid-cols-2 gap-x-6 gap-y-1 text-xs">
              {[["Liquidity", 82], ["Profitability", 78], ["Efficiency", 71], ["Solvency", 68]].map(([k, v]) => (
                <div key={String(k)} className="flex justify-between min-w-[100px]">
                  <span className="text-ud-text-muted">{k}</span>
                  <span className="font-mono tabular-nums font-medium text-ud-text-primary">{v}</span>
                </div>
              ))}
            </div>
          </div>
        </Section>

        <Section title="Tax deadlines" subtitle="Upcoming TRA filings">
          <div className="space-y-2.5">
            {TAX_FILINGS.filter((t) => t.status === "Pending" || t.status === "Upcoming" || t.status === "Overdue").slice(0, 3).map((t) => (
              <div key={t.id} className="p-3 rounded-xl border border-ud-border bg-white">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <div className="text-sm font-semibold text-ud-text-primary">{t.type} — {t.period}</div>
                    <div className="text-xs text-ud-text-muted mt-0.5">Due {formatDate(t.dueDate)}</div>
                  </div>
                  <Badge variant={t.status === "Overdue" ? "danger" : t.status === "Pending" ? "warning" : "info"} pulse={t.status === "Overdue"}>
                    {daysUntil(t.dueDate) >= 0 ? `${daysUntil(t.dueDate)}d left` : `${Math.abs(daysUntil(t.dueDate))}d overdue`}
                  </Badge>
                </div>
                <div className="mt-2 font-mono tabular-nums text-sm font-bold text-ud-text-primary">
                  <CurrencyDisplay amount={t.amount} />
                </div>
              </div>
            ))}
          </div>
        </Section>

        <Section title="Alerts" subtitle="Worth your attention">
          <div className="space-y-2">
            {ALERTS.map((a) => {
              const Icon = a.icon;
              return (
                <Link
                  key={a.title}
                  href={a.href}
                  className="block p-3 rounded-xl border border-ud-border hover:border-ud-primary bg-white transition-colors"
                >
                  <div className="flex items-start gap-2.5">
                    <div className={
                      a.type === "danger" ? "p-1.5 rounded-lg bg-ud-danger-bg text-ud-danger" :
                      a.type === "warning" ? "p-1.5 rounded-lg bg-ud-warning-bg text-ud-warning" :
                      "p-1.5 rounded-lg bg-ud-info-bg text-ud-info"
                    }>
                      <Icon className="w-3.5 h-3.5" />
                    </div>
                    <div className="min-w-0">
                      <div className="text-sm font-medium text-ud-text-primary">{a.title}</div>
                      <div className="text-xs text-ud-text-muted">{a.description}</div>
                    </div>
                  </div>
                </Link>
              );
            })}
          </div>
        </Section>
      </div>

      {/* Floating AI button */}
      <Link
        href="/ai-assistant"
        className="fixed bottom-20 right-4 md:bottom-6 md:right-6 z-30 w-14 h-14 rounded-full gradient-teal flex items-center justify-center shadow-elevated hover:scale-105 transition-transform"
        aria-label="Open AI Assistant"
      >
        <div className="absolute inset-0 rounded-full bg-ud-primary-glow opacity-0 animate-pulse-soft" />
        <Sparkles className="w-6 h-6 text-white relative z-10" />
      </Link>
    </PageWrapper>
  );
}

function Section({
  title,
  subtitle,
  children,
  className,
  headerRight,
}: {
  title: string;
  subtitle?: string;
  children: React.ReactNode;
  className?: string;
  headerRight?: React.ReactNode;
}) {
  return (
    <motion.section
      initial={{ opacity: 0, y: 14 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.4 }}
      className={`bg-white rounded-2xl border border-ud-border p-5 shadow-card ${className ?? ""}`}
    >
      <div className="flex items-start justify-between mb-4">
        <div>
          <h3 className="font-display font-bold text-base text-ud-text-primary">{title}</h3>
          {subtitle && <p className="text-xs text-ud-text-muted mt-0.5">{subtitle}</p>}
        </div>
        {headerRight}
      </div>
      {children}
    </motion.section>
  );
}
