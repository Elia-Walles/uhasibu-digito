  ...
  return (
    <PageWrapper>
      ...
      <Modal
        ...
        footer={<><Button variant="ghost" onClick={() => setOpen(false)}>{t("Cancel")}</Button><Button variant="primary" onClick={() => { setOpen(false); toast.success(t("Payment recorded")); }}>{t("Save")}</Button></>}
      >
        <div className="space-y-3">
          ...
          <Input label={t("Reference")} placeholder={t("Transaction reference")} />
          <Input label={t("Date")} type="date" defaultValue={new Date().toISOString().split("T")[0]} />
        </div>
      </Modal>
    </PageWrapper>
  );
