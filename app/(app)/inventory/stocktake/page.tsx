  ...
  return (
    <PageWrapper>
          ...
          <div className="bg-white border border-ud-border rounded-2xl p-6 shadow-card">
            {step === 0 && (
              <div className="space-y-4 max-w-md">
                <h3 className="font-display font-bold text-lg">Stocktake setup</h3>
            ...
            {step === 2 && (
              <div className="space-y-3">
                <h3 className="font-display font-bold text-lg">Enter counts</h3>
                <div className="overflow-x-auto rounded-xl border border-ud-border max-h-96 overflow-y-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-ud-surface-2 sticky top-0">
                      <tr>
                        <th className="text-left px-3 py-2 text-xs uppercase tracking-[0.06em]" scope="col">Item</th>
                        <th className="text-right px-3 py-2 text-xs uppercase tracking-[0.06em]" scope="col">System qty</th>
                        <th className="text-right px-3 py-2 text-xs uppercase tracking-[0.06em] w-32" scope="col">Counted</th>
                      </tr>
                    </thead>
                    <tbody>
            ...
            {step === 3 && (
              <div className="space-y-3">
                <h3 className="font-display font-bold text-lg">Review variances</h3>
                <div className="p-4 rounded-xl bg-ud-warning-bg border border-ud-warning/20 text-sm text-ud-warning">
                  3 items have variances. Review carefully before posting.
                </div>
                <p className="text-sm text-ud-text-muted">Adjustments will be posted to the General Ledger as an inventory adjustment journal entry.</p>
              </div>
            )}
            {step === 4 && (
              <div className="text-center py-8">
                ...
                <h3 className="font-display font-bold text-xl">Ready to post</h3>
                <p className="text-sm text-ud-text-muted mt-1">All counts captured. Click below to post the stocktake.</p>
              </div>
            )}
          </div>
        </motion.div>
      ...
      <div className="flex justify-between mt-6">
        <Button variant="outline" disabled={step === 0} onClick={() => setStep(Math.max(0, step - 1))}>Back</Button>
        {step < 4 ? (
